...
{
public:
  ExtractTask(size_t id, SentenceAlignment &sentence,PhraseExtractionOptions &initoptions, Moses::OutputFileStream &extractFile, Moses::OutputFileStream &extractFileInv,Moses::OutputFileStream &extractFileOrientation,vector<string> sbsWordsAug): //P5-1 modify augument
    m_sentence(sentence),
...
{
/*ofstream outFile("/home/nlp",ios:app);
if(!outFile)
	throw incalid_argument("not open");
outFile<<"=============fuck"===========<<endl;
outFile.close();*/
 ...
 //P1 END
  ...
  while(true) {
    ...
    //get seg sentence variable
     ...
     if(sbsFileP){
       cout<<"sbsFileP get data success========="<<endl;
      }else{
         cout<<"fail  fail failed==========="<<endl;
      }
   //P3 END
   ...
   //get word of seg
	...
	if (sentence.create( englishString, foreignString, alignmentString, weightString, i, false)) {
   	...
   	ExtractTask *task = new ExtractTask(i-1, sentence, options, extractFile , extractFileInv, extractFileOrientation,sbsWords);
        //p5-4end
      task->Run();
      delete task;
...
void ExtractTask::extract(SentenceAlignment &sentence)
{
  ...
  ofstream outfile("/home/hadoop/test");
 
 // bool TestFile=file1.open("file1")
...
//for(int str=0;str<sbsWordsUse.size();str++){
 for(int startE=0;startE<countE;startE++){
     // int startE=0;
      // startE=atoi(sbsWordsUse[str].c_str());
    for(int endE=startE;
        (endE<countE && (relaxLimit || endE<startE+m_options.maxPhraseLength));
        endE++) {
      
      // outfile<<"startE=="<<startE<<"endE=="<<endE<<endl;
      int minF = 9999;
      int maxF = -1;
          ...
          for(int startF=minF;
            ...
            for(int endF=maxF;
              ...
              if(buildExtraStructure) { // phrase || hier
                ...
                //m1
                for(int str=0;str<sbsWordsUse.size();str++){
                  boundL=atoi(sbsWordsUse[str].c_str());
                   boundR=atoi(sbsWordsUse[str++].c_str());
                if(startF<=boundL&&endF>=boundR){
                if(sentence.alignedToT[startE].size()!=0&&sentence.alignedToT[endE].size()!=0){ 
