...
{
 // ofstream outfile;
   //outfile.open("/home/hadoop/lexoutput",ios::app);
   //***this is caculate phrase translation table lexical weighting score
  // lexical translation probability
  double lexScore = 1.0;
  ...
  for(size_t ti=0; ti<alignment.alignedToT.size(); ti++) {
    ...
    if (srcIndices.empty()) {
     ...
     // outfile<<"unaligned word target=="<<vcbT.getWord(phraseT[ti])<<endl;
      ...
      for (set< size_t >::const_iterator p(srcIndices.begin()); p != srcIndices.end(); ++p) {
       // outfile << "ti ==" <<ti <<endl; 
       // outfile<<"phraseS[*p]=="<<phraseS[ *p ]<<","<<phraseT[ ti ]<<endl;
       // outfile<<"Source"<<vcbS.getWord(phraseS[*p])<<",target"<<vcbT.getWord(phraseT[ti])<<endl;
        thisWordScore += lexTable.permissiveLookup( phraseS[ *p ], phraseT[ ti ] );     
        //outfile<<"thisWordScore(else)=="<<thisWordScore<<endl;
      }
      lexScore *= thisWordScore / (double)srcIndices.size();
      // outfile <<"lexScore(else)=="<<lexScore<<"srcIndices.size()=="<<srcIndices.size()<<endl;
