...
{
public:
  ExtractTask(size_t id, SentenceAlignment &sentence,PhraseExtractionOptions &initoptions, Moses::OutputFileStream &extractFile, Moses::OutputFileStream &extractFileInv,Moses::OutputFileStream &extractFileOrientation,vector<string> sbsWordsAug,vector<string> tbsWordsAug)://P5-1 modify augument
    m_sentence(sentence),
    m_options(initoptions),
    m_extractFile(extractFile),
    m_extractFileInv(extractFileInv),
    //P5-3
    sbsWordsUse(sbsWordsAug),
    tbsWordsUse(tbsWordsAug),
...
private:
  ...
  //P5-2
  vector< string > sbsWordsUse;
  vector< string > tbsWordsUse;
...
int main(int argc, char* argv[])
{
 ...
 if (argc < 6) {
  ...
  //P1
  //input seg information
  //need absolute path
  const char *const fileNameSBS=getenv("SBS");
  const char *const fileNameTBS=getenv("TBS");

  Moses::InputFileStream sbsFile(fileNameSBS);
  Moses::InputFileStream tbsFile(fileNameTBS);

  istream *sbsFileP = &sbsFile;
  istream *tbsFileP = &tbsFile;
  ...
  while(true) {
    ...
    //P2
    //get seg sentence variable
    char sbsString[LINE_MAX_LENGTH];
    char tbsString[LINE_MAX_LENGTH];
    ...
    //P3
    //get seg sentence variable
    SAFE_GETLINE((*sbsFileP), sbsString, LINE_MAX_LENGTH, '\n', __FILE__);
    SAFE_GETLINE((*tbsFileP), tbsString, LINE_MAX_LENGTH, '\n', __FILE__);
    ...
    //P4
    //get word of seg
    vector<string> sbsWords= tokenize( sbsString );
    vector<string> tbsWords= tokenize( tbsString );
	...
	//P5-4
	//modify ExtracTask argument
   	ExtractTask *task = new ExtractTask(i-1, sentence, options, extractFile , extractFileInv, extractFileOrientation,sbsWords,tbsWords);
      task->Run();
      delete task;
  ...
  for(int startE=0; startE<countE; startE++) {
    for(int endE=startE;
      ...
      if (maxF >= 0 && // aligned to any source words at all
		...
		//P6
		char *strExtractLoose=getenv("EXTRACT_LOOSE");//stat str that extract loose
		char *strExtractMatch=getenv("EXTRACT_MATCH");//stat str that extract match
		int extractLoose=0;//stat number that extract loose
		int extractMatch=0;//stat number that extract match
		char *strS=getenv("LIMIT_S");//limit boundary that within FR phrase
		char *strT=getenv("LIMIT_T");//limit boundary that within EN phrase
		int limitS=0;//limit boundary number that within FR phrase
		int limitT=0;//limit boundary number that within EN phrase
		int iterS=0;//S boundary index
		int iterT=0;//T boundary index
		int countLimitS=0;//boundary count  that within FR phrase
		int countLimitT=0;//boundary count  that within EN phrase
		int leftBoundaryF=-1;//initial
		int rightBoundaryF=99999;//initial
		int mosesExtractLenF=0;//initial
		int matchLenF=0;//initial
		int leftBoundaryE=-1;//initial
		int rightBoundaryE=99999;//initial
		int mosesExtractLenE=0;//initial
		int matchLenE=0;//initial

		sscanf(strExtractLoose,"%d",&extractLoose);
		sscanf(strExtractMatch,"%d",&extractMatch);
		sscanf(strS,"%d",&limitS);
		sscanf(strT,"%d",&limitT);

		if(extractLoose==1){//process loose
			for(size_t iter=0;iter<sbsWordsUse.size();iter++){
				sscanf(sbsWordsUse[iter].c_str(),"%d",&iterS);
				if(iterS>=startF && iterS<endF)
					countLimitS++;
				if(countLimitS==limitS+1)
					break;
			}

			for(size_t iter=0;iter<tbsWordsUse.size();iter++){
				sscanf(tbsWordsUse[iter].c_str(),"%d",&iterT);
				if(iterT>=startE && iterT<endE)
					countLimitT++;
				if(countLimitT==limitT+1)
					break;
			}
		

			if(countLimitS<=limitS && countLimitT<=limitT)
                		addPhrase(sentence, startE, endE, startF, endF, orientationInfo);
		}
		else if(extractMatch==1){//process match
			for(size_t iter=0;iter<sbsWordsUse.size();iter++){
				sscanf(sbsWordsUse[iter].c_str(),"%d",&iterS);
				if(iterS>=startF && iterS<endF)
					countLimitS++;
			...
			if(countLimitS==limitS && countLimitT==limitT){
				for(size_t iter=0;iter<sbsWordsUse.size();iter++){
					sscanf(sbsWordsUse[iter].c_str(),"%d",&iterS);
					if(iterS<startF)
						if(iterS>leftBoundaryF)
							leftBoundaryF=iterS;
					if(iterS>=endF)
						if(iterS<rightBoundaryF)
							rightBoundaryF=iterS;
				}
				
				for(size_t iter=0;iter<tbsWordsUse.size();iter++){
					sscanf(tbsWordsUse[iter].c_str(),"%d",&iterT);
					if(iterT<startE)
						if(iterT>leftBoundaryE)
							leftBoundaryE=iterT;
					if(iterT>=endE)
						if(iterT<rightBoundaryE)
							rightBoundaryE=iterT;
				}

				mosesExtractLenF=(endF-startF)+1;
				matchLenF=rightBoundaryF-leftBoundaryF;
				mosesExtractLenE=(endE-startE)+1;
				matchLenE=rightBoundaryE-leftBoundaryE;

				if(mosesExtractLenF==matchLenF && mosesExtractLenE==matchLenE)
					addPhrase(sentence, startE, endE, startF, endF, orientationInfo);
			}
